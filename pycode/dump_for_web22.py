...
# -*- coding: utf-8 -*-
"""
Štíhlejší dump „důležitých“ souborů webové appky:
- Pravidla zahrnutí/vyloučení jako dřív, navíc head+tail výpis velkých textů.
- Speciální zkrácení pro index.html (inline var book = {...}) a book.json (souhrn).
- Soubor 'files' se nesype celý, ale jen statistika a pár ukázek.
...
# Automatické vyloučení (přidány extra položky)
...
# Nové „extra“ vyloučení (typicky ukecané/duplicitní)
EXTRA_AUTO_EXCLUDE_EXTS = {
    ".map",   # sourcemapy JS/CSS
    # přidej klidně ".log" apod. dle potřeby
}
EXTRA_AUTO_EXCLUDE_BASENAMES = {
    # pokud chceš, můžeš vyřadit i 'combined.css' apod.
    # "combined.css",
}

# Co NAOPAK zahrnout (pro logiku webu)
INCLUDE_EXTS = {
...
# Výstupní režimy
PRINT_TREE = True
PRINT_CONTENT = True          # když False, vytiskne jen strom
MAX_FILES_IN_CONTENT = None   # např. 200 (None = bez omezení)
...
def _write_header(fh, root: Path, ex_dirs: Set[Path], ex_files: Set[Path]) -> None:
    ...
    if EXTRA_AUTO_EXCLUDE_EXTS or EXTRA_AUTO_EXCLUDE_BASENAMES:
...
def _summarize_inline_book(html_text: str) -> tuple[str, str | None]:
    """
    Najde 'var book = {...};' a vrátí (upravený_html, info).
    JSON je v JS, ale bývá validní – zkusíme ho převést a spočítat statistiky.
    """
    m = BOOK_VAR_RE.search(html_text)
    if not m:
        return html_text, None
    json_raw = m.group(2)
    # pokus o normalizaci JSONu (odstranění koncových čárek apod. neděláme; obvykle je validní)
    info = None
    try:
        book = json.loads(json_raw)
        slots = book.get("slots", [])
        # posbírej URL z "options"
        def iter_urls(obj):
...
def _summarize_book_json(text: str) -> str:
    try:
        ...
        return text  # když to není validní JSON, nechej jak je (případně se uplatní head+tail)
    slots = obj.get("slots", [])
    def iter_urls(o):
    ...
    # Krátký náhled prvních ~HEAD_BYTES znaků
    preview = text[:min(len(text), HEAD_BYTES)]
    return (
        "<<<BOOK.JSON SUMMARY>>>\n"
    ...
    for f in files:
        ...
        try:
            with f.open("rb") as rf:
                sample = rf.read(min(st.st_size, MAX_TEXT_BYTES_PER_FILE))
        except Exception:
            continue

        is_bin = _is_probably_binary(sample[:2048])
        ...
        if SUMMARIZE_INDEX_INLINE_BOOK and name == "index.html":
            ...
            fh.write("<<<BEGIN TEXT (truncated head+tail)>>>\n")
            fh.write(_head_tail_text(sample))
            fh.write("<<<END TEXT>>>\n\n")
